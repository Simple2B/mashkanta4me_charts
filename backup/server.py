...
def home():
    ...
    for rule in app.url_map.iter_rules():
        if "GET" in rule.methods and has_no_empty_params(rule):
            ...
            if (
                rule.endpoint != "home"
                and rule.endpoint != "update_data_route"
...
def analytics():
    with open(
        "./static/json_files/change_in_monthly_return_as_function_of_first_payment.json"
    ) as f_change_monthly, open(
        "./static/json_files/loan_cost_as_function_of_monthly_payment.json"
    ) as f_loan_cost, open(
        "./static/json_files/Principal_halved_function_of_monthly_payment.json"
    ) as f_principal_halved:
...
def InterestFile_Prime():
    with open("./static/json_files/InterestFile_Prime.json") as f:
        data_set = json.load(f)
    return render_template(
        "home.html", data_set=data_set, filter_form="economic_expectations_line"
    )
...
def InterestFile_Variable_W_CPI():
    with open("./static/json_files/InterestFile_Variable_W_CPI.json") as f:
        data_set = json.load(f)
    return render_template(
        "home.html", data_set=data_set, filter_form="economic_expectations_bar"
    )
...
def historical():
    with open("./static/json_files/real_historical.json") as f_real, open(
        "./static/json_files/nominal_historical.json"
    ) as f_nominal:
        data_set_real = json.load(f_real)
        data_set_nominal = json.load(f_nominal)
    return render_template(
        "home.html",
        data_set={"real": data_set_real, "nominal": data_set_nominal},
        filter_form="historical",
    )
