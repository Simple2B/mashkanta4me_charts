...
color_palette = ['rgba(255, 107, 101, 1)', 'rgba(255, 131, 101, 1)', 'rgba(255, 148, 101, 1)',
                     'rgba(255, 167, 25, 1)', 'rgba(255, 203, 25, 1)', 'rgba(52, 216, 153, 1)',
                     'rgba(52, 203, 216, 1)', 'rgba(52, 172, 216, 1)', 'rgba(16, 163, 255, 1)',
                     'rgba(121, 52, 216, 1)']
...
def interest(df):
    ...
    for ltv_idx, (ltv_string, d) in enumerate(zip(ltv_strings, dicts), 1):
        ...
        data = [{'x': int(data_point[1]['Years']), 'y': data_point[1]['Interest_rate'],
                 'bank': data_point[1]['Bank_name'], 'ltv': ltv_string, 'ltv_tooltip': ltv_tooltip[ltv_strings.index(data_point[1]['LTV'])]}
...
def interest_by_bank(df):
    ...
    for bank_idx, (lbl, d) in enumerate(zip(label, dicts), 1):
        ...
        data = [{'x': int(data_point[1]['Years']), 'y': data_point[1]['Interest_rate'],
                 'bank': lbl, 'ltv': ltv_id[ltv_strings.index(data_point[1]['LTV'])],
...
def loan_cost_as_function_of_monthly_payment(df):
    config = {'label': ['עד 1.25', 'עד 1.5', 'עד 1.75', 'עד 2', 'מעל 2'],
...
def change_in_monthly_return_as_function_of_first_payment(df):
    config = {'label': ['עד 10%', 'עד 20%', 'עד 30%', 'עד 40%', 'מעל 40%'],
...
def Principal_halved_function_of_monthly_payment(df):
    config = {'label': ['עד 5 שנים', 'בין 5 ל- 10 שנים', 'בין 10 ל- 15 שנים', 'בין 15 ל- 20 שנים', 'מעל 20 שנים'],
...
def analytics(df, config):
    ...
    for danger_idx, d in enumerate(dicts, 1):
        ...
        data = [{'x': data_point[1]['mortgage_size'].item(), 'y': data_point[1]['monthly_payment'].item(), 'danger': data_point[1][config['col_header']].item()}
                for data_point in curr_points.iterrows()]
        for key in keys:
            if key != 'data':
...
def InterestFile(df, sheet):
    if sheet['plot_type'] == 'Line':
        ...
        dicts = [{'data': data, 'pointRadius': 5, 'fill': False, 'borderColor': 'rgba(255, 202, 25, 1)',
                  'backgroundColor': 'rgba(255, 202, 25, 1)', 'name': sheet['name']}]
    else:
...
def real_historical(df):
    config = {'label': ['שנה', 'שנתיים', '3 שנים', '4 שנים', '5 שנים', '7 שנים', '10 שנים', '20 שנים'],
              'header': 'ריבית ריאלית'}
    return historical_interest(df, config)


def nominal_historical(df):
    config = {'label': ['שנה', 'שנתיים', '3 שנים', '4 שנים', '5 שנים', '7 שנים', '10 שנים', '15 שנים'],
              'header': 'ריבית נומינלית'}
    return historical_interest(df, config)


def historical_interest(df, config):
    keys = ['label', 'header', 'backgroundColor', 'borderColor', 'pointRadius', 'fill', 'data']
    label = config['label']
    years = len(label)
    ...
    for years_idx, d in enumerate(dicts, 1):
        ...
        data = [{'x': data_point[1].strftime('%m-%Y'), 'y': t[1]} #strftime('%Y-%m-%dT%H:%M:%SZ')} #.strftime('%m-%Y')}
...
def excel_to_data_sets():
    ...
    excel_files = [f for f in EXCEL_FILES_DIR.iterdir() if f.is_file()
                   and not f.stem.startswith('.') and not f.stem.startswith('~')]
    ...
    for excel_file in excel_files:
        ...
        if excel_file.stem == 'InterestFile':
            sheets = [{'name': 'Prime', 'plot_type': 'Line', 'col': 'B', 'cell_range': [3, 362]},
                      {'name': 'Inflation', 'plot_type': 'Line', 'col': 'B', 'cell_range': [3, 362]},
                      {'name': 'Variable WO CPI', 'plot_type': 'Bar', 'col': 'B', 'cell_range': [3, 8]},
                      {'name': 'Variable W CPI', 'plot_type': 'Bar', 'col': 'B', 'cell_range': [3, 8]}]
            for sheet in sheets:
                df = pd.read_excel(file_full_path,
                ...
                file_json = excel_file.stem + '_' + sheet['name'].replace(' ', '_') + '.json'
                with open(pathlib.Path(JSON_FILES_DIR, file_json), 'w') as f:
                    f.write(df_as_json)
